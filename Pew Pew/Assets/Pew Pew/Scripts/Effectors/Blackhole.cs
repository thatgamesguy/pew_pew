...
namespace GameCore
{
    ...
    public class Blackhole : MonoBehaviour, HitListener, HitDeathInvoker
    {
        ...
        void Update()
        {
            ...
            Grid.Instance.ApplyImplosiveForce((float)Mathf.Sin(sprayAngle / 2) * .15f * .4f, transform.position, 2.5f);
        }
        ...
        private void CheckDamage(int damageAmount, bool cameraShake)
        {
            ...
            {
                Grid.Instance.ApplyExplosiveForce(8f, transform.position, 3f);
                Kill(cameraShake);
            }
            else
            {
                Grid.Instance.ApplyExplosiveForce(4f, transform.position, 3f);
