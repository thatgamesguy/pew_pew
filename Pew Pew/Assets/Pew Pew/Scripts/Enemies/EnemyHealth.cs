...
namespace GameCore
{
    ...
    public class EnemyHealth : MonoBehaviour, HitListener, BombListener, HitDeathInvoker
    {
        ...
        private void CheckDamage(int damageAmount, bool cameraShake)
        {
            ...
            {
                Grid.Instance.ApplyExplosiveForce(8f * explosiveForceMulti, transform.position, 1.8f);
                Kill(cameraShake);
            }
            else
            {
                Grid.Instance.ApplyExplosiveForce(8f * explosiveForceMulti * 0.5f, transform.position, 1.8f);
                if (audioOnDamage)
                {
