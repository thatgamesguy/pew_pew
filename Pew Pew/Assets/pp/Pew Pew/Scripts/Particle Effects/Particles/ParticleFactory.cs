...
namespace PE2D
{
	...
	public class ParticleFactory : MonoBehaviour
	{
        ...
        private PauseHandler _pauseStatus;
		...
		public bool ParticlesRemaining ()
		{
			for (int i = _particleArray.Start; i < _particleArray.Count; i++) {
				if (_particleArray [i].percentLife > 0f) {
					return true;
				}
		...
		public List<CustomParticle> GetLiveParticles ()
		{
			var aliveParticles = new List<CustomParticle> ();

			for (int i = _particleArray.Start; i < _particleArray.Count; i++) {
				if (_particleArray [i].percentLife > 0f) {
					aliveParticles.Add (_particleArray [i]);
				}
			}
		...
		void Update ()
		{
            if (_pauseStatus != null && _pauseStatus.isPaused) { return; }
